...
void MainWindow::createActions()
{
  ...
  file_menu->addAction(tr("New DBC File"), [this]()
                       { newFile(); }, QKeySequence::New);
  file_menu->addAction(tr("Open DBC File..."), [this]()
                       { openFile(); }, QKeySequence::Open);
  ...
  {
    load_opendbc_menu->addAction(dbc_name, [this, name = dbc_name]()
                                 { loadDBCFromOpendbc(name); });
  }

  file_menu->addAction(tr("Load DBC From Clipboard"), [=]()
                       { loadFromClipboard(); });
  ...
  view_menu->addSeparator();
  view_menu->addAction(tr("Reset Window Layout"), [this]()
  ...
  if (!can->liveStreaming() && std::exchange(car_fingerprint, can->carFingerprint()) != car_fingerprint)
  {
    ...
    if (!dbc()->nonEmptyDBCCount() && !car_fingerprint.isEmpty())
    {
      ...
      if (dbc_name != QJsonValue::Undefined)
      {
        ...
        QTimer::singleShot(0, this, [dbc_name, this]()
...
void MainWindow::updateLoadSaveMenus()
{
  ...
  {
    if (source >= 64)
      continue; // Sent and blocked buses are handled implicitly

    SourceSet ss = {source, uint8_t(source + 128), uint8_t(source + 192)};

    QMenu *bus_menu = new QMenu(this);
    bus_menu->addAction(tr("New DBC File..."), [=]()
                        { newFile(ss); });
    bus_menu->addAction(tr("Open DBC File..."), [=]()
                        { openFile(ss); });
    bus_menu->addAction(tr("Load DBC From Clipboard..."), [=]()
                        { loadFromClipboard(ss, false); });
    ...
    if (auto dbc_file = dbc()->findDBCFile(source))
    {
      ...
      bus_menu->addAction(tr("Save..."), [=]()
                          { saveFile(dbc_file); });
      bus_menu->addAction(tr("Save As..."), [=]()
                          { saveFileAs(dbc_file); });
      bus_menu->addAction(tr("Copy to Clipboard..."), [=]()
                          { saveFileToClipboard(dbc_file); });
      bus_menu->addAction(tr("Remove from this bus..."), [=]()
                          { closeFile(ss); });
      bus_menu->addAction(tr("Remove from all buses..."), [=]()
                          { closeFile(dbc_file); });
