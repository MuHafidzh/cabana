...
int main(int argc, char *argv[])
{
  ...
  if (SocketCanStream::available())
  {
    ...
    cmd_parser.addOption({"socketcan-fd", "enable CAN FD for SocketCAN"});
    cmd_parser.addOption({"socketcan-brs", "enable Bit Rate Switch for SocketCAN (requires CAN FD)"});
  }
  ...
  else if (cmd_parser.isSet("socketcan"))
  {
    ...
    // Parse CAN FD and BRS options
    config.enableCanFD = cmd_parser.isSet("socketcan-fd");
    config.enableBRS = cmd_parser.isSet("socketcan-brs");

    // BRS requires CAN FD
    if (config.enableBRS && !config.enableCanFD)
    {
      qWarning() << "Warning: BRS requires CAN FD. Enabling CAN FD automatically.";
      config.enableCanFD = true;
    }
    ...
    {
      qWarning() << "Failed to create SocketCAN stream:" << e.what();
      return 0;
    }
  }
  else
  {
    ...
    if (cmd_parser.isSet("dcam"))
      replay_flags |= REPLAY_FLAG_DCAM;
    if (cmd_parser.isSet("no-vipc"))
  ...
  if (stream)
  {
    ...
    if (!dbc_file.isEmpty())
    {
      // Add delay for DBC loading to prevent race condition
      QTimer::singleShot(1000, [&w, dbc_file]()
                         { w.loadFile(dbc_file); });
    }
  }
  else
